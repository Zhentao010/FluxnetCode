# -*- coding: utf-8 -*-
"""
Created on Sat Nov 26 12:46:15 2022
...
def DTRSWC(filename):          #读取昼夜温差，VPD和土壤含水量
    ...
    for i in range(3):
        if ('SWC_F_MDS_'+str(i+1)) in dtrswc.columns:
            for j in range(m):
                data[j][i] = dtrswc['SWC_F_MDS_' + str(i+1)][j];
    for i in range(m):
        data[i][3] = dtrswc['GPP_NT_VUT_REF'][i]
...
path1 = 'C:/Users/Lenovo/Desktop/未划分年份的初级数据/原始数据SWC_GPP/';    #储存读取的DTR_VPD_SWC数据的位置
path2 = 'C:/Users/Lenovo/Desktop/SWC_GPP_Slope.xlsx'    #储存计算的相关系数的位置
slope = np.zeros((1,3));
...
for csv_file in os.listdir(path):
    ...
    data_pd.to_excel(writer1, sheet_name='SWC——GPP');
    writer1.save();

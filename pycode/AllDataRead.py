...
def dataTransfer(fileN):
    ...
    ecom = len(eco);
    Data = np.zeros((ecom,5),dtype = float);
    n=0;
    for i1 in range(ecom):
        if eco['RECO_NT_VUT_REF'][i1] != -9999:
            Data[n][0] = eco['TIMESTAMP_START'][i1];
            Data[n][1] = eco['TA_F_MDS'][i1];
            Data[n][2] = eco['RECO_NT_VUT_REF'][i1];
            Data[n][3] = 1000/(eco['TA_F_MDS'][i1]+273);
            Data[n][4] = np.log(eco['RECO_NT_VUT_REF'][i1]);
            n = n+1;
        else:
            continue;
    return Data
...
for csv_file in os.listdir(path):
    a = csv_file[4:10];
    path1 = 'C:/Users/Lenovo/Desktop/ALLvalue/' + a + '.xlsx';
